...
void GPIOC_callback_init(void (*handler)(void))
{
	gpio_C_callback = handler;
}

void GPIOA_callback_init(void (*handler)(void))
{
...
uint8_t GPIO_read_pin(gpio_port_name_t port_name, uint8_t pin)
{
	...
	return_value = GPIO_read_port(port_name) >> pin;	/* Read the whole port */
	return (uint8_t) (return_value & 1 );
}
